...
当前仓库里的日报由 `npm run pipeline` 生成：采集新闻、调用 OpenAI 抽取结构化信号、聚合首页图表数据、再生成 PDF。没有 API key 或模型响应不合规时，流水线会按批次降级到 `deterministic`，保证部署不中断。

The committed report is generated by `npm run pipeline`: collect news, call OpenAI for structured extraction, aggregate dashboard chart data, then render the PDF. If the API key is missing or a model response fails schema validation, the pipeline falls back batch-by-batch to `deterministic` so deployment remains reproducible.
