...
function metric(doc: PDFKit.PDFDocument, label: string, value: string, x: number, y: number, width: number) {
  doc
    .roundedRect(x, y, width, 56, 6)
    .fillAndStroke("#f8fafc", "#d9e1ea");
  doc
    .font("Report")
    .fontSize(7.5)
    .fillColor("#64748b")
  ...
  doc
    ...
    .fontSize(17)
    .fillColor("#111827")
    .text(value, x + 10, y + 27, { width: width - 20 });
}

function topEvent(doc: PDFKit.PDFDocument, rank: number, score: number, title: string, body: string) {
  const startY = doc.y;
  doc
    .roundedRect(doc.page.margins.left, startY, doc.page.width - doc.page.margins.left - doc.page.margins.right, 72, 6)
    .fillAndStroke("#ffffff", "#e5e7eb");
  doc
    .font("Report")
    .fontSize(16)
    .fillColor("#0b6bcb")
    .text(String(rank), doc.page.margins.left + 12, startY + 12, { width: 28 });
  doc
    .font("Report")
    .fontSize(10)
    .fillColor("#111827")
    .text(title, doc.page.margins.left + 48, startY + 12, {
  ...
  doc
    ...
    .fontSize(8.5)
    .fillColor("#64748b")
    .text(body, doc.page.margins.left + 48, startY + 36, {
  ...
  doc
    ...
    .fontSize(16)
    .fillColor("#168a3a")
    .text(String(score), doc.page.width - doc.page.margins.right - 42, startY + 14, {
      width: 36,
      align: "right"
    });
  doc.y = startY + 82;
...
function generatePdf(report: DailyReport) {
  ...
  doc
    .fontSize(21)
    .fillColor("#ffffff")
    .text("AI 舆情分析日报", 42, 38);
  doc
    .fontSize(11)
    .fillColor("#b8b8bf")
    .text("Daily AI Insight Report", 42, 66);
  doc
    .fontSize(8.5)
    .fillColor("#9b9ba1")
    .text(
