...
function section(doc: PDFKit.PDFDocument, title: string) {
  ...
  doc.font("Helvetica").fontSize(14).fillColor("#111827").text(pdfText(title));
  doc
...
function paragraph(doc: PDFKit.PDFDocument, text: string, options: PDFKit.Mixins.TextOptions = {}) {
  doc.font("Helvetica").fontSize(9.5).fillColor("#374151").text(pdfText(text), {
    lineGap: 3,
    ...options
  });
}
...
function metric(doc: PDFKit.PDFDocument, label: string, value: string, x: number, y: number, width: number) {
  ...
  doc.font("Helvetica").fontSize(7.5).fillColor("#64748b").text(pdfText(label.toUpperCase()), x + 10, y + 10, {
    width: width - 20
  });
  doc.font("Helvetica").fontSize(17).fillColor("#111827").text(pdfText(value), x + 10, y + 27, {
    width: width - 20
  });
}
...
function topEvent(doc: PDFKit.PDFDocument, rank: number, score: number, title: string, body: string) {
  ...
  doc.font("Helvetica").fontSize(10).fillColor("#111827").text(pdfText(title), doc.page.margins.left + 48, startY + 12, {
    width: doc.page.width - doc.page.margins.left - doc.page.margins.right - 110,
    lineGap: 2
  });
  doc.font("Helvetica").fontSize(8.5).fillColor("#64748b").text(pdfText(body), doc.page.margins.left + 48, startY + 36, {
...
export function renderDailyReportPdf(report: DailyReport) {
  return new Promise<Buffer>((resolve, reject) => {
    ...
    doc.fontSize(21).fillColor("#ffffff").text("Daily AI Insight Report", 42, 38);
    ...
    section(doc, "Deep Dives");
    for (const item of report.deepDives) {
      ensureSpace(doc, 74);
      doc.fontSize(10.5).fillColor("#111827").text(pdfText(item.headline), { lineGap: 2 });
    ...
    for (let index = 0; index < pageCount; index += 1) {
      ...
      doc
        ...
        .fillColor("#8a92a0")
        .text(
          `Copyright 2026 Kkertin. All rights reserved. - Contact: kkertin1214@gmail.com - ${index + 1}/${pageCount}`,
          42,
          doc.page.height - 30,
